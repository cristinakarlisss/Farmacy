...
namespace Farmacy
{
    public class SQL_Connection
    {
        ...
        public bool LoadPreview(ReportViewer report, int venta, BindingSource bindingSource, decimal importe)
        {
            using(SqlConnection cnn = new SqlConnection(conexion))
            {
                bool ok = false;
                try
                {
                    ...
                    SqlCommand cmd = new SqlCommand("SELECT B.Nombre as Product,Count(B.Nombre)As Qty,A.Precio as Price, Count(B.Nombre) * A.Precio AS Total,D.[UserName] as [User],C.Subtotal as Subtotal, " +
                        $"C.IGV as IVA, C.Total AS TotalVenta, C.Fecha as Fecha, {importe} as Importe, ({importe} - C.Total) as Cambio FROM Productos_Vendidos A " +
                        ...
                        "GROUP BY B.Nombre, A.Precio, C.Subtotal, C.IGV, C.Total, D.UserName, C.Fecha", cnn) { CommandType = CommandType.Text };
                    SqlDataAdapter da = new SqlDataAdapter(cmd);
                    DataSet dataSet = new DataSet();
                    try
                    {
